...
class Graph:
    """
    Neo4j queries often only use one CPU core on the database server.
    This simple wrapper can make it handle a defined maximum of concurrent
    queries. It's not very nice, but it could speed things up.
    """

    def __init__(self, *a, concurrent_queries=1, **k):
        """
        Get database connectors for the amount defined in concurrent_queries.
        Do a poor man's subclass.

        :param a: positional arguments for py2neo.Graph()
        :param concurrent_queries: number of concurrent queries and database
                                   connections
        :param k: keyword arguments for py2neo.Graph()
        """
        ...
        self.__dict__.update(
            {parent_attribute: self.graph.__getattribute__(parent_attribute)
             ...
             and not parent_attribute in ['data', 'run']}
        )

    def data(self, *a, **k):
        """
        Does the same things as py2neo.Graph.data, only returns Futures
